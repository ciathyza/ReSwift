...
public protocol StoreType: DispatchingStoreType
{
	...
	/**
	 ...
	 - note: Subscriptions are not ordered, so an order of state updates cannot be guaranteed.
	 */
	func subscribe<SelectedState, S: StoreSubscriber>(_ subscriber:S, transform:((Subscription<State>) -> Subscription<SelectedState>)?) where S.StoreSubscriberStateType == SelectedState
	...
	associatedtype AsyncActionCreator = (_ state:State, _ store:StoreType, _ actionCreatorCallback:(ActionCreator) -> Void) -> Void
}
