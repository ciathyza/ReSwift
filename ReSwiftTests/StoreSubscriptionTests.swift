...
store.subscribe(subscriber!)
XCTAssertEqual(store.subscriptions.flatMap({
$0.subscriber
}).count, 1)
...
$0.subscriber
	...
	func testNewStateModifyingSubscriptionsDoesNotDiscardNewSubscription()
	{
		...
		let subscriber2 = BlockSubscriber<TestAppState>
		{
			_ in
			self.store.dispatch(SetValueAction(2))
		}
		
		let subscriber1 = BlockSubscriber<TestAppState>
		{
